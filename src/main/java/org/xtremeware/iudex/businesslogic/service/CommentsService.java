...
public class CommentsService extends CrudService<CommentVo> {
     ...
     */
    public List<CommentVo> getByCourseId(DataAccessAdapter em, long courseId) {

        List<CommentVo> vos = ((JpaCommentDao) getDao()).getByCourseId(em, courseId);
     ...
     */
    protected CrudDao<CommentVo, ?> getDao() {
     ...
     */
    public void validateVo(DataAccessAdapter em, CommentVo vo) throws InvalidVoException, ExternalServiceConnectionException {
    ...
    public CommentVo create(DataAccessAdapter em, CommentVo vo) throws InvalidVoException, MaxCommentsLimitReachedException, ExternalServiceConnectionException {
        validateVo(em, vo);
     ...
     *
     * @param em data access adapter
     * @param userId id of the user
     * @return number of comments submitted on the current day
     */
    public int checkUserCommentsCounter(DataAccessAdapter em, Long userId) {
        return ((JpaCommentDao) getDao()).getUserCommentsCounter(em, userId);
    }

    public CommentVo update(DataAccessAdapter em, CommentVo vo) throws InvalidVoException, ExternalServiceConnectionException {
        validateVo(em, vo);
    ...
    public void remove(DataAccessAdapter em, long id) {
        List<CommentRatingVo> ratings = getDaoFactory().getCommentRatingDao().getByCommentId(em, id);

        for (CommentRatingVo rating : ratings) {
            getDaoFactory().getCommentRatingDao().remove(em, rating.getId());
        }
        getDao().remove(em, id);
    }

    public CommentVo getById(DataAccessAdapter em, long id) {
