...
public class CommentsFacade extends AbstractFacade {
    ...
    public CommentVo createComment(CommentVo vo) throws
        ...
        try {
            ...
            transaction.begin();
            createdVo = getServiceFactory().createCommentsService().create(
                    entityManager,
                    vo);
            transaction.commit();
    ...
    public void deleteComment(long commentId) throws DataBaseException {
        ...
        try {
            ...
            transaction = entityManager.getTransaction();
            transaction.begin();
            getServiceFactory().createCommentsService().delete(entityManager,
                    commentId);
            transaction.commit();
        } catch (Exception exception) {
            getServiceFactory().getLogService().error(exception.getMessage(),
                    exception);
            FacadesHelperImplementation.checkExceptionAndRollback(entityManager,
