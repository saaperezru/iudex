...
public class CoursesFacade extends AbstractFacade {
	...
	/**
	 * Searches for courses by professor and professor name that contain the
	 * string specified in the query parameter. If the query is empty or
	 * full of spaces, the results will be empty. (Because of the overhead
	 * of bringing ALL the information from database and translating it to
	 * the proper format).
	 *
	 ...
	 * 
	 * @param professorId
	 * @return If there is no professor associated with the provided id an
	 * empty list will be returned
	 */
	public List<CourseVoVwFull> getByProfessorId(long professorId) {
		EntityManager em = null;
		List<CourseVoVwFull> list = new ArrayList<CourseVoVwFull>();
		HashMap<Long, SubjectVoVwSmall> subjectsVoVws = new HashMap<Long, SubjectVoVwSmall>();
		try {
			...
			RatingSummaryVo subjectRating = getServiceFactory().createProfessorRatingsService().getSummary(em, professorId);
			ProfessorVo s = getServiceFactory().createProfessorsService().getById(em, professorId);
			ProfessorVoVwSmall professor = new ProfessorVoVwSmall(s.getId(), s.getFirstName() + " " + s.getLastName(), subjectRating);
			for (CourseVo c : courses) {
				if (!subjectsVoVws.containsKey(c.getSubjectId())) {
					RatingSummaryVo rating = getServiceFactory().createSubjectRatingsService().getSummary(em, c.getSubjectId());
					SubjectVo p = getServiceFactory().createSubjectsService().getById(em, c.getSubjectId());
					subjectsVoVws.put(p.getId(), new SubjectVoVwSmall(p.getId(), p.getName(), rating));
				}
				list.add(new CourseVoVwFull(c, subjectsVoVws.get(c.getSubjectId()), professor));
			}
