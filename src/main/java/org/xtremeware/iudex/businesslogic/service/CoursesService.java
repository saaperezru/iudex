...
public class CoursesService extends CrudService<CourseVo, CourseEntity> {
    ...
    public void validateVo(EntityManager em, CourseVo course) throws InvalidVoException {
        ...
        if (getDaoFactory().getProfessorDao().getById(em, course.getProfessorId()) == null) {
            throw new InvalidVoException("Long professorId in the provided "
                    + "CourseVo must correspond to an existing professor entity in the database");
        }
        if (getDaoFactory().getSubjectDao().getById(em, course.getSubjectId()) == null) {
            throw new InvalidVoException("Long subjectId in the provided "
                    + "CourseVo must correspond to an existing subject entity in the database");
        }
        if (getDaoFactory().getPeriodDao().getById(em, course.getPeriodId()) == null) {
            throw new InvalidVoException("Long periodId in the provided "
                    + "CourseVo must correspond to an existing period entity in the database");
        }
        if (course.getRatingCount() < 0) {
            throw new InvalidVoException("Long ratingCount in the provided "
                    + "CourseVo must be greater than one");
        }
        if (course.getRatingAverage() < 0) {
            throw new InvalidVoException("Double ratingAverage in the provided "
                    + "CourseVo must be greater than one");
        }
    }
    ...
    public List<CourseVo> getSimilarCourses(EntityManager em, String professorName, String subjectName, Long preiodId) throws ExternalServiceConnectionException {
    ...
    @Override
    public CourseEntity voToEntity(EntityManager em, CourseVo vo) throws InvalidVoException,
